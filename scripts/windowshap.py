...
class DynamicWindowSHAP():
    ...
    def shap_values(self, num_output=1, nsamples_in_loop='auto', nsamples_final='auto'):
        ...
        while flag:
            ...
            # Demographic SHAP values
            if self.num_dem_ftr > 0:
                dem_phi = shap_values[0, :, :self.num_dem_ftr]
            else:
                dem_phi = None

            # Time-series SHAP values
            ts_shap_values = shap_values[:, :, self.num_dem_ftr:]
            n_ts_windows_shap = ts_shap_values.shape[2]

            # Stop splitting if max window count reached
            ...
            for i in range(self.num_ts_ftr):
                ...
                for j in range(self.num_window[i]):
                    ...
                    # Handle single-window (n_w = 1) case safely
                    if n_ts_windows_shap == 1:
                        phi_test = abs(ts_shap_values[0, 0, 0])
                    else:
                        idx = sum(self.num_window[:i]) + j
                        if idx >= n_ts_windows_shap:
                            continue
                        phi_test = abs(ts_shap_values[0, 0, idx])

                    if phi_test > self.delta:
                        if j == 0:
                            S.add(int(self.split_points[i][j] / 2))
                        else:
                            S.add(int((self.split_points[i][j-1] + self.split_points[i][j]) / 2))

                if set(S) != set(self.split_points[i]):
                    flag += 1
                    self.split_points[i] = sorted(S)

        # === Final SHAP computation with larger nsamples ===
        self.explainer = shap.KernelExplainer(self.wraper_predict, self.background_data)
        ...
        ts_shap_values = shap_values[:, :, self.num_dem_ftr:]
        n_ts_windows_shap = ts_shap_values.shape[2]
        single_window = (n_ts_windows_shap == 1)

        # Assigning SHAP values to each time step
        ...
        for i in range(self.num_ts_ftr):
            for j in range(self.num_window[i]):
                ...
                width = max(1, end_ind - start_ind)

                if single_window:
                    phi_ij = np.mean(ts_shap_values[0, :, 0])
                else:
                    idx = sum(self.num_window[:i]) + j
                    if idx >= n_ts_windows_shap:
                        continue
                    phi_ij = np.mean(ts_shap_values[0, :, idx])

                ts_phi[0, start_ind:end_ind, i] = phi_ij / width
