...
def heat_map(start, stop, x, shap_values, var_name='Feature 1', plot_type='bar', title=None):
    ...
    # Define bins and normalize, forcing 0 to be part of the colorbar
    if np.allclose(shap_values, 0):
        # Handle all-zero SHAP: create tiny bounds around 0
        bounds = np.array([-1e-6, 0, 1e-6])
    else:
        # Normal case
        bounds = np.arange(np.min(shap_values), np.max(shap_values) + 1e-6, 0.005)
        # Ensure 0 is in the bounds
        if np.min(shap_values) > 0 or np.max(shap_values) < 0:
